...
- **`broadcast(event)`** — called after key mutations in `server/routes.ts` (messages, project updates, change orders, estimates, invoices, chat messages). Sends to filtered recipients: same companyId, explicit clientUserId, or members listed in allowedUserIds (subcontractors/notaries assigned to the project).
